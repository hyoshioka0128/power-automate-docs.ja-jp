...
*Power Automate には継続的に新しい機能が追加されているため、クラシック ワークフロー機能と同等かそれ以上に優れています。Power Automate が機能を獲得したら、このテーブルの情報を更新します; 頻繁に確認してください! クラシック ワークフローをフローに置き換えるのに役立つ今後のフロー機能については、2019 年 4 月のリリース ノートの [Power Automate の新機能と予定されている機能](https://docs.microsoft.com/business-applications-release-notes/April19/microsoft-flow/planned-features) を参照してください!*
...
フローでは、シナリオをサポートする Power Automate 機能の一部を説明するチュートリアルの後半でで示されているように、これはより簡単に構築できます。 これには次のものが含まれます。
...
1. トリガーが構成されたら、フローで実行するアクションを追加します。 これにより、見積品目と値を識別するために必要な集計の詳細が承認者に提供されます。 まず、**Common Data Service (現在の環境) - レコードの一覧表示** アクションを追加します。 見積もりから個々の品目を取得する必要があるため、エンティティを **見積明細行** に設定します。 フローがトリガーされた見積もりに属する見積品目のみを確実にリストするには、OData スタイル フィルター条件を指定します。 **フィルター クエリ** フィールドに、*\_quoteid_value eq* と入力し、表示される動的な値のリストから *見積もり* を選択します。
...
これで承認の構造が作成されたので、承認者は次の手順で決定をおこなうために必要なすべての情報を得られました。 承認を要求するオンデマンド フローの完全な例を以下に示します。
...
-   **フローはどのくらいの頻度でトリガーできますか?**
    ...
    -   直接 API アクセスと同様に、システムにはスロットル/制限があります。全文についてはこちら: <https://docs.microsoft.com/flow/limits-and-config>
