...
Power Apps にはいくつかの種類のプロセスがあり、それぞれ異なる目的のためにデザインされています。  
...
プロセスと同様に、ビジネス ルールとベスト プラクティスも作成できます。 詳細:[フォームにロジックを適用するために業務ルールおよびビジネス レコメンデーションを作成する](/powerapps/maker/model-driven-apps/create-business-rules-recommendations-apply-logic-form)  

> [!NOTE]
> プロセスの使用は、Power Apps  およびフローに対するライセンス要件に影響する場合があります。 詳細: [エンティティのライセンス要件](https://docs.microsoft.com/powerapps/maker/common-data-service/data-platform-entity-licenses) 
...
お使いの環境には、一般的な営業、サービス、マーケティングタスク用の、ほとんど変更せずにすぐ使用できる種々の業務プロセス フローが含まれます。 あるいは、独自に作成できます。 詳細: [業務プロセス フローの作成](create-business-process-flow.md)  
  
<a name="BKMK_WF"></a>   
## <a name="when-to-use-workflows"></a>ワークフローを使用する場合

舞台裏でビジネス プロセスを自動化するためにワークフローを使用します。 ワークフローはシステム イベントによって通常開始されるため、実行されていることを認識する必要はありません。 バックグラウンドで動作するワークフローは「非同期」です。 ワークフローは、注文の発送時に確認メールを顧客に自動的に送信するなど、一般的なタスクを自動化するときに人が手動で開始するように構成することもできます。 リアルタイムに動作するワークフローは「同期」です。 詳細情報: [ワークフロー プロセス](workflow-processes.md)  
...
- [ソリューション エクスプローラー](/powerapps/maker/model-driven-apps/advanced-navigation#solution-explorer) を開いて、**コンポーネント** > **プロセス** に移動します。 このパスは、カスタマイズ ツールでそのほかのカスタマイズ作業を行うときに便利なアクセスを提供します。  

- **[設定](/powerapps/maker/model-driven-apps/advanced-navigation#settings)** > **プロセス** このパスを使用すると、すべてのカスタム ビューを含むプロセス エンティティに対して定義したビューを使用できます。  
...
システム管理者、システム カスタマイザー、最高経営責任者のセキュリティ ロールを持つユーザーだけが、環境全体に適用されるプロセスを作成できます。 他のロールを持つユーザーは、制限されたアクセス レベルでプロセスを作成できます。 たとえば、ユーザー アクセス レベルのユーザーは、所有するレコードで自身が使用するためのワークフローを作成できます。  
...
|**条件の確認**|ワークフロー、アクション|論理的な "if \<condition> then" 構文。<br /><br /> ワークフローが実行中のレコード、N:1 の関連付けでそのレコードにリンクされているいずれかのレコード、またはこれまでの手順によって作成された任意のレコードの値をチェックできます。 条件が `true` のとき、これらの値に基づき追加の手順を定義できます。|  
|**条件分岐**|ワークフロー、アクション|理論的な "else-if-then" 構文。 エディターはテキスト “Otherwise, if \<condition> then:” を使用します<br /><br /> 以前に定義したチェック条件を選択し、チェック条件が `false` を返すときに、追加のステップを定義する条件分岐を追加できます。|  
|**既定の操作**|ワークフロー、アクション|論理的な "else" ステートメント。 エディターはテキスト “Otherwise:” を使用します。<br /><br /> 以前定義したチェック条件、条件分岐、待機状態、または並列待機分岐を選択します。条件要素や分岐要素で定義した基準に適合しないすべてのケースに対して、既定のアクションを使用して手順を定義できます。|  
