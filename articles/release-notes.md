...
- flowmaker
- enduser
ms.openlocfilehash: b107a8545383c7f51649d2b02f7be9b711a63157
ms.sourcegitcommit: b043b7e8c29afee4f4f25bbf0d5a662d9af9272c
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 12/09/2020
ms.locfileid: "4710047"
---
...
2. 高度な条件または式はどのようにして使えばよいですか。
   ...
   * クラウド フローに複数のケースが必要な場合は、既存の条件内から **条件の追加** を選択します。
...
- **Microsoft Teams コネクタの機能強化** - チャネルに新しいメッセージがあるときにクラウド フローを実行できるようにする、Microsoft Teams 用の新しいトリガー。
...
- **Outlook Web App での Microsoft To-Do のクラウド フローの作成** - Outlook Web App で \@ メンションされたユーザーには、クラウド フローを作成するためのショートカットが表示されます。 このフローでは、メールのコンテンツに基づき、Microsoft To-Do で \@言及されたユーザーのタスクが自動的に作成されます。
...
- **エラーの詳細分析** - クラウド フロー内で発生するすべてのエラーに関する情報が掲載された新しい埋め込みレポートです。
- **Office 365 グループとのフローの共有** - Office 365 の最新のグループをクラウド フローの所有者にすることができます。また、グループ内のすべてのユーザーがフローを実行できるように、Office 365 グループとボタン フローを共有することができます。
...
- **クラウド フローから Power Apps にデータを返す** - Power Apps でビルドされたアプリから呼び出すことができるフローを構築して、データをアプリに返すことができます。 視覚的なドラッグ アンド ドロップのフロー デザイナーを使用して、アプリに必要なロジックを構築できます。 
...
- **Dataverse での承認** - 最新バージョンの Dataverse に最新の承認が構築されます。 つまり、Common Data Service コネクタとの間で送受信する承認の状態を読み取るフローを構築することができます。
...
- **フローのボタンの詳細表示** - 共有されているクラウド フローを実行すると、そのフローで使用されるすべてのアクションが表示されるようになりました。
...
- **共同編集通知** - 自分が所有しているクラウド フローが共同作業者によって変更されると、メール通知が届き、変更者と変更されたフローが伝えられます。
...
すべてのリージョンで、Power Automate の起動パネルを利用できるようになりました。 このパネルでは、SharePoint のリストまたはドキュメント ライブラリ内で実行するときに、クラウド フローに値を追加することができます。
...
- **OneDrive for Business の統合** - [OneDrive for Business 内のクラウド フロー ボタン](https://flow.microsoft.com/blog/microsoft-flow-integration-in-one-drive-for-business-and-new-connector-actions/) を使って、選択したファイルまたはフォルダーに対するフローを作成またはトリガーできるようになりました。
...
- **最初のリリースでの SharePoint のより深い統合** - レビュー フロー用の新しい「標準」送信と、初期リリース テナントのクラウド フローを実行するときに入力を収集するためのフロー パネルがあります。
...
- **フロー履歴のダウンロード** - クラウド フローの実行履歴を CSV ファイルとしてダウンロードし、Excel で開きます。
...
* **フローの削除に関する情報** - クラウド フローを削除する場合、そのフローに対して保留中の実行すべてが一覧表示されます。
...
* **実行の詳細を示すウィンドウ** - クラウド フローが失敗した場合に、フローを修正するための有効な手順を含む新しいウィンドウが右側に表示されます。
...
* **フローの失敗に関する電子メール** - クラウド フローが失敗すると、電子メールによる通知が届きます。 このような失敗に関する電子メールは、週に 1 回だけ送信されます。ユーザー側でメールの配信のオンまたはオフを設定できます。
...
* **整数および浮動小数点の変数** - 特定のロジック セットを実行する回数をカウントするために、クラウド フロー実行内のカウンターを初期化し、インクリメントまたはデクリメントします。
* **フローの詳細ページ** - **マイ フロー** 一覧からクラウド フローを選択すると、アクセス権を持つユーザー、実行履歴など、そのフローの詳細情報が表示されたページが開きます。
...
* **SharePoint ドキュメント ライブラリの統合** - ドキュメント ライブラリの任意のファイルを選択して、クラウド フローを開始できます。たとえば、承認を得るためにマネージャーに送信したり、[さらに多くのことを実行できます](https://flow.microsoft.com/blog/flow-in-spo-document-libraries/)。
...
* **Power Apps コミュニティ プラン** - Power Apps コミュニティ プランは、個人が Power Apps、Power Automate、およびDataverse のスキルを探索、学習、および構築するための無料のプランです。
...
* **チーム フローが標準で利用可能** - チーム フローが標準で利用可能になり、複数のユーザーがクラウド フローとチーム フローを同時に所有して管理できるようになりました。
* **Power Automate のコネクターの構築** - だれでも自分の Power Automate コネクターを無料で送信して、世界中で使用することができます。
* **”無駄を省いた”デザイナー** - 特定のテンプレートでは、新しいバージョンのデザイナーにより、クラウド フローの作成に必要なフィールドのみが表示されるようになり、操作が簡略化されます。
...
* **フローのコピーを作成する** - フローのコピーを作成して、下書きバージョンで作業したり、過去に作成したクラウド フローを複製したりできるようになりました。
...
* **ホーム画面からボタンをトリガーする** - モバイル デバイスのホーム画面またはロック画面からのフロー ボタンへのショートカットにより、これまでよりも素早くクラウド フローをトリガーできます。
...
* **チーム フロー** - チーム フローを使うと、複数のユーザーが共同でクラウド フローを所有および管理でき、だれかが組織を離れた場合でも、作成したフローは引き続き実行できます。
...
* **実行の再送信** - 失敗したクラウド フローを修正して再び実行したい場合は、失敗した実行を再送信できます。
* **実行のキャンセル** - クラウド フローが動かなくなった場合、実行を明示的にキャンセルできるようになりました。
...
* **デザイナーの機能強化** - パフォーマンスが向上したほか、すべてのアクションで「...」メニューから接続を更新および修正できるようになりました。また、クラウド フローの実行を終了するための終了と呼ばれる新しいステップが追加されました。
...
* **HTTP エンドポイントとしてのクラウド フローの使用** - クラウド フロー自体を HTTP API として使用できるようになりました。 フロー内には "要求" と呼ばれるトリガーがあり、"応答" カードを追加することで、着信要求に応答できます。
...
* **完成前のフローのテスト** - 開始アクションを実行した場合、クラウド フローを保存するたびに、フローの実行結果をページ内で直接確認できます。
...
* クラウド フローをカスタマイズした後、ギャラリーに [独自のテンプレートを発行](publish-a-template.md) することができます。
* フローのチェックと実行すべてについて、履歴を確認することができます。
* クラウド フローを保存するときに、トリガー アクションを実行するだけで、[実行中のフローを即座に確認](see-a-flow-run.md) することができます。
