...
受信トレイで新規メールを受診した際など、多くのトリガーはイベントに基くフローをすぐに開始できます。 このようなトリガーは便利ですが、データソース内の項目のプロパティに基づいて特定のアクションを実行し、定義されているスケジュールでデータソースを照会するフローを実行したい場合もあります。 これを行うには、フローをスケジュール (1日1回など) で開始し、**それぞれに適用する**などのループアクションを使用して項目の一覧を処理することができます。 たとえば、**それぞれに適用する** を使用すると、データベースのレコードや、Microsoft SharePoint の項目一覧を更新できます。

このチュートリアルでは、15分ごとに実行されるフローを作成し、以下のことを行います :
